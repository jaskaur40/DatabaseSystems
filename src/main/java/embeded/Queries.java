 ...
 * <p>
 * Help taken from stackoverflow.com, neo4j.com, informit.com, tutorialspoint.com, youtube.com
 */
public class Queries {
    ...
    /**
     * Find all Game of Thrones characters in the database
     */
    public void runQueryForGOTCharacters() {
        System.out.println("\n\nQUERY: Find all gotCharacters in the database\n");
    ...
    public void getAverageRating() {
        ...
        while (gotCharacters.hasNext()) {
            ...
            for (Relationship r : relationships) {
                Integer stars = (Integer) r.getProperty("stars");
                totalStars += stars;
                rCount++;
            }
            System.out.println("   " + gotChar.getProperty("name") + ", Viewers: " +
                    rCount + ", Average rating: " +
                    (float) totalStars / rCount);
        }
    }
    ...
    public void getCharactersAUserHasLiked() {
        ...
        while (users.hasNext()) {
            ...
            for (Relationship r : user.getRelationships(
                ...
                Node gotCharacter = r.getOtherNode(user);
    ...
    public void gotCharactersThatMyFriendsHaveLiked() {
        for (int i = 0; i < 5; i++) {
            ...
            Traverser traverserObj = myFriends.traverse(person);
            System.out.println(username + "'s friends: ");
            for (Node friend : traverserObj.nodes()) {
                System.out.println("\t" + friend.getProperty("username"));
            }


            //
            traverserObj = getFriendsLikingTraverser(person);

            System.out.println("GotCharacters that " + username + " friends have liked: ");
            for (Node gotCharacter : traverserObj.nodes()) {
                System.out.println("\t" + gotCharacter.getProperty("name"));
            }
        }
    }
    ...
    /**
     * Provides the traverser object based on what my friends have liked.
     *
     * @param person
     * @return
     */
    private Traverser getFriendsLikingTraverser(Node person) {
        Traverser traverser;
        TraversalDescription gOTCharactersThatFriendsLike = graphDB.traversalDescription()
                ...
                .expand(new PathExpander<Object>() {
                    ...
                    public Iterable<Relationship> expand(Path path,
                                                         BranchState<Object> branchState) {
                        // Get the depth of this node
                        int depth = path.length();

                        if (depth == 0) {
                            // Depth=0 refers to the user's node (starting node)
